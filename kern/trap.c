...
trap_init(void)
{
	...
	// seteo las interrupciones segun la tabla intel
	// formato trap_N

	extern void trap_0();  // divido 0

	extern void trap_1();  // debug (single step)

	extern void trap_2();  // non maskable interrup (NMI)

	extern void trap_3();  // breakpoint

	extern void trap_4();  // Overflow

	extern void trap_5();  // Bound range exceeded

	extern void trap_6();  // ivnalid opcode

	extern void trap_7();  // device not available

	extern void trap_8();  // double fault

	extern void trap_10();  // Invalid Task State Segment

	extern void trap_11();  // Segment not present

	extern void trap_12();  // Stack Segment Fault

	extern void trap_13();  // General Protection Fault

	extern void trap_14();  // Page fault

	extern void trap_16();  // x87 Floating Point Exception

	extern void trap_17();  // Alignment Check

	extern void trap_18();  // Machine Check

	extern void trap_19();  // SIMD Floating-Point Exception

	extern void trap_32();  // Timer interrup

	extern void trap_33();  // Keyboard interrup

	extern void trap_34();  // Serial interrup

	extern void trap_48();  // Syscall
...
trap_init_percpu(void)
{
	...
	// Obtengo numero de cpu
	int id = cpunum();
	// Obtengo CpuInfo del cpu
	struct CpuInfo *cpu = &cpus[id];
	// Obtengo Taskstate del CPU
	...
	// Calculo el indice del taskk segment
...
trap_dispatch(struct Trapframe *tf)
{
	...
	case T_SYSCALL: {
		int32_t val_ret = syscall(tf->tf_regs.reg_eax,
		                          tf->tf_regs.reg_edx,
		                          tf->tf_regs.reg_ecx,
		                          tf->tf_regs.reg_ebx,
		                          tf->tf_regs.reg_edi,
		                          tf->tf_regs.reg_esi);
		tf->tf_regs.reg_eax = val_ret;
		return;
	}
	}
...
page_fault_handler(struct Trapframe *tf)
{
	...
	// LAB 3: Your code here.
	if ((tf->tf_cs & 3) == 0) {
		panic("page_fault_handler: page fault in ring 0");
	}
	...
	if (curenv->env_pgfault_upcall != NULL) {
		...
		// chequeo si es una llamada recursiva
		if (tf->tf_esp >= (UXSTACKTOP - PGSIZE) &&
		    tf->tf_esp < UXSTACKTOP) {
			// si es recursivo se debe dejar un extra word de 4
			// bytes entre el UTrapframe nuevo y el viejo
			...
			// Si no es recursiva se escribe en UXSTACKTOP
		...
		// seteo los valores de UTrapframe copiando los valores de tf
